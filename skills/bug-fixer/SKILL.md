...
[第一性原则]
    ...
    **反复失败就停**：同一个 bug 反复修了多次还没修好，Agent 应该停下来重新审视问题本身——可能不是代码层面的问题，可能是架构问题、环境问题或理解偏差。

**⚠️ 量化停止标准（必须遵守）**：
- **同一修复方案连续失败 3 次** → 停止，重新审视假设
- **不同修复方案累计失败 5 次** → 停止，升级为架构问题，记录到 docs/流水线/异常记录/ 并等待人工介入
- **单次调试总耗时超过 30 分钟** → 暂停，输出当前进度和建议，询问用户是否继续
- **每次暂停都必须**：输出已尝试的方案清单、失败原因、下一步建议
