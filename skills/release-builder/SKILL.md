...
[发布检查清单]
    ...
    [性能门禁]
        ...
        **产物大小**：
        | 产物类型 | 目标大小 | 最大大小 |
        |---------|---------|---------|
        | 单个 JS chunk | ≤ 200KB | ≤ 500KB |
        | 单个 CSS 文件 | ≤ 50KB | ≤ 100KB |
        | 首页总加载 | ≤ 500KB | ≤ 1MB |
        | 构建产物总大小 | ≤ 50MB | ≤ 100MB |

        **检查方式**：
        - 分析构建产物大小：`du -sh [BUILD_DIR]/` 和 `ls -lhS [BUILD_DIR]/static/chunks/ | head -20`
        - 如有 next-bundle-analyzer → 生成 bundle 分析报告
        - 超过最大大小 → 🔴 阻断发布，建议拆分或优化
        - 超过目标大小 → 🟡 警告，建议优化

        **运行时性能**（需启动 dev server）：
        - 首页加载时间 ≤ 3 秒
        - API 响应时间 ≤ 500ms
        - 无内存泄漏迹象
    ...
    [文档生成]
        ...
        **CHANGELOG.md**（如不存在则创建）：
        ...
        - 修复问题列表
        - 破坏性变更（如有）
        - 数据来源：git log --oneline [last-tag]..HEAD
