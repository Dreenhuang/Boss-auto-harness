...
[开发规则清单]
    ...
    [代码规范]
        ...
        - 单函数不超过 50 行，超了拆子函数
        - 函数圈复杂度不超过 10（推荐），硬限制不超过 20
        - 函数参数不超过 4 个，超了用 options 对象
        - 嵌套深度不超过 4 层
    ...
    [复杂度量化指标]
        ...
        | 单文件行数 | ≤ 300 | ≤ 500 | wc -l |
        | 单函数行数 | ≤ 50 | ≤ 80 | 代码审查 |
        | 函数圈复杂度 | ≤ 10 | ≤ 20 | ESLint complexity |
        | 函数参数个数 | ≤ 4 | ≤ 6 | ESLint max-params |
        | 嵌套深度 | ≤ 3 | ≤ 4 | ESLint max-depth |
        | 认知复杂度 | ≤ 15 | ≤ 25 | SonarJS |
        | 回调嵌套 | ≤ 2 | ≤ 3 | ESLint max-nested-callbacks |

        项目初始化时自动生成 .eslintrc.quality.js（模板来自 quality-gate Skill）。
...
[Phase 完成度判断]
    ...
    **四步走**（必须全部通过才能确认 Phase 完成）：
    ...
    第一步：Quality Gate（自动化质量门禁）
    - 调用 quality-gate Skill 执行完整检查流水线
    - lint、typecheck、安全扫描、复杂度检查、构建验证
    - 全部通过才进入第二步
    - 输出证据：quality-gate 报告

    第二步：测试完整性
    - 调用 test-runner Skill 执行单元测试 + 集成测试
    - 该 Phase 计划的所有功能都已实现
    - 无遗漏、无半成品
    - 输出证据：test-runner 报告 + 功能清单打勾
    ...
    - 输出证据：编译命令输出 + 启动日志 + API 响应 + 设计数值对比结果
...
[工作流程（持续开发模式）]
    ...
    [Phase 执行流程]
        ...
        第二步：逐个 Task 实现 + 单 Task Review 循环
            ...
            开发后——对照验证 + Quality Gate + Test + Review 循环：
            7. 读取代码实际值，逐项与设计数值核对，有偏差则修正
            8. 对照 Product-Spec.md 确认功能行为符合描述
            9. 调用 quality-gate Skill 执行自动化质量检查（lint + typecheck + 安全扫描 + 复杂度检查）
               quality-gate 不通过 → 自动修复或调用 bug-fixer → 重新 quality-gate
            10. 调用 test-runner Skill 执行单元测试
                单元测试不通过 → 调用 bug-fixer 修复 → 重新 test-runner
