...
└── .claude/
    ...
    │   └── git-committer.md           # Git 提交 Agent 🆕
    ├── EVOLUTION.md                   # 进化引擎
    ├── feedback/                      # 经验教训
    └── skills/
        ├── product-spec-builder/      # 需求收集（含结构化确认清单+风险预判+问题跳过+竞品分析+市场扫描+用户画像+决策验证）🆕🆕🆕🆕🆕
        │   └── templates/
        │       ├── product-spec-template.md
        │       ├── changelog-template.md
        │       └── competitor-analysis-template.md  # 🆕 竞品分析模板
        ...
        ├── bug-fixer/                 # Bug 修复
        ├── code-review/               # 代码审查（五大维度+返工机制）🆕
        ├── deploy-engine/             # 多目标部署
        ├── pipeline-engine/           # Pipeline DAG编排
...
| **agent-orchestrator** | **.claude/agents/agent-orchestrator.md** | **多Agent协作编排** | **智能分析项目特征+选择最优Agent组合+动态调整协作模式** |
...
- WebSearch 搜索行业现状、市场规模、发展趋势
- 生成市场扫描报告 → 归档至 docs/需求/市场扫描报告.md
- 向用户展示关键发现，确认市场机会和定位
- ⚠️ **此步骤调用 product-spec-builder 的 [市场环境扫描] 流程**（详见 product-spec-builder/SKILL.md 第746-784行）

**步骤2：竞品分析（强制执行，不可跳过）**
- 🗣️ 向用户说明："现在进行竞品分析，搜索3-5个主要竞品"
- WebSearch 搜索目标领域的竞品
- 生成竞品分析简报（功能对比、技术架构、用户体验、定价策略）
- 归档至 docs/需求/竞品分析报告.md
- 差异化追问：基于竞品分析，向用户提问差异化策略
- ⚠️ **此步骤调用 product-spec-builder 的 [竞品分析阶段] 流程**（详见 product-spec-builder/SKILL.md 第668-744行）

**步骤3：用户画像构建（强制执行，不可跳过）**
- 🗣️ 向用户说明："现在构建目标用户画像"
- 基础属性收集：年龄、职业、地域、技术能力
- 行为特征刻画：使用场景、使用频率、偏好习惯
- 痛点挖掘：核心痛点、现有解决方案的不足
- 生成用户画像 → 归档至 docs/需求/用户画像.md
...
- 用户确认方式：
  - "确认" / "可以" / "开始" → **创建 .prd-confirmed 状态文件 + 记录确认时间戳** → 标记PRD已确认，进入第二阶段
  - "需要修改" → 回到对应步骤调整
  - "/跳过" → 标记为强制待确认，再次建议不跳过

**📌 PRD确认状态持久化机制（必须执行）**
- ✅ **用户确认PRD后**：必须创建 `.prd-confirmed` 文件（无内容即可），记录确认时间到 `docs/需求/PRD确认记录.md`
...
- 🔴类问题（必须确认）：强烈建议不跳过，坚持跳过则标记为"强制待确认"，在PRD确认时再次提醒
...
- ⚠️ **此提示与 product-spec-builder SKILL.md 第483-500行的模式转换提示为同一逻辑的不同表述**，Boss模式中以本处为准，product-spec-builder单独调用时以该文件为准
- 输出此提示后，进入第二阶段

**第二阶段：规划与开发（全自动化，无需用户确认）**

**⭐ 步骤0：初始化 Agent Orchestrator（必须执行）**
- 🗣️ 向用户输出："🔄 正在分析项目特征，选择最优Agent协作模式..."
- 执行项目画像分析（类型、规模、复杂度、阶段）
- 运行决策算法，选择最优Agent组合模式
- 🗣️ 输出："✅ Agent组合就绪！当前模式：[模式名] | 参与Agent：[列表]"
- 初始化Agent实例，建立通信通道
- 生成 docs/流水线/Agent组合初始化记录.md

1. 有 Spec 无 Plan → 自动启动 dev-planner（检查待确认清单中的相关项）
2. 有 Plan 无代码 → 自动启动 dev-builder（由 Orchestrator 分配任务给 Implementer）
3. 有代码 → 自动继续下一个 Phase
