...
async function createTables() {
  console.log('🏗️  Creating tables via Supabase API...')
  ...
  const createStatements = [
    `CREATE TABLE IF NOT EXISTS users (
      ...
      preferences JSONB DEFAULT '{"interests":[],"experience":"零基础","goals":[],"timeSlot":"","notificationEnabled":true}'::jsonb,
    ...
    if (error?.message?.includes('relation')) {
      try {
        ...
        if (checkError?.message?.includes('relation')) {
          console.log(`  ⚠️  ${tableName}: Could not create via API (needs Dashboard SQL)`)
        } else {
          created++
  ...
  return created === createStatements.length
}
...
async function runSeed() {
  ...
  if (postsError) console.error('  ⚠️  Posts:', postsError.message)
  ...
  if (pathsError) console.error('  ⚠️  Paths:', pathsError.message)
  ...
  if (stepsError) console.error('  ⚠️  Steps:', stepsError.message)
  ...
  if (hotError) console.error('  ⚠️  Hot searches:', hotError.message)
  ...
  if (msgError) console.error('  ⚠️  Messages:', msgError.message)
...
async function main() {
  ...
  console.log('\n🏗️  Creating tables...')
  const allCreated = await createTables()
  
  if (!allCreated) {
    console.log('\n⚠️  Some tables could not be created via API.')
    console.log('\n📋 Please execute SQL migration manually:')
    console.log('1. Open: https://supabase.com/dashboard/project/jaduaifzmgvaotyqnjfe/sql')
    console.log('2. Click "New query"')
    console.log('3. Copy and paste content from: database/migration.sql')
